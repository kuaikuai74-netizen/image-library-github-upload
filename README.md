...
超级管理员登录后，顶部会出现“管理后台”入口，可访问 `/admin` 查看总览看板、运营报表、用户与权限、下载记录、上传任务、素材组治理、质量检查、审计日志、公告管理、文档中心、在线留言、策略配置、存储健康、系统自检和部署清单。当前后台已支持新建用户、编辑用户、启用/禁用用户、重置密码、发布/编辑公告，以及新建/编辑使用文档。普通用户可在素材库左侧导航查看有权限的已发布文档，也可进入“在线留言”提交需求并查看自己的历史留言。
...
- `/messages` 允许已登录用户提交最多 2000 字的在线留言，并且只显示本人历史记录；超级管理员可在后台查看全部用户留言，面板每 10 秒自动刷新。
...
- 在线留言：展示所有用户提交的留言、提交人和时间，每 10 秒自动刷新；普通用户仅能读取自己的留言。
...
用户管理、公告、文档操作和批量下载会写入审计或下载批次记录。相关迁移包括 `20260723003000_admin_user_audit_actions`、`20260723004500_announcements_documents`、`20260723011000_download_batches` 和 `20260902052300_online_messages`。部署包含该功能的版本前，需要运行 `npm run db:deploy`。
...
- `GET /api/messages` / `POST /api/messages` 读取或提交当前用户的在线留言。
- `GET /api/admin/messages` 查看全部在线留言，仅限超级管理员。
