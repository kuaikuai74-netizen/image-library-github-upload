...
export function AssetLibrary({ currentUser }: AssetLibraryProps) {
  ...
  const [columns, setColumns] = useState<4 | 6 | 8>(6);
    ...
    <div className="library-app">
      <LibraryHeader
        ...
        onUpload={() => {
          ...
          if (!assetGroup) return showNotice("当前筛选范围没有可上传的素材组。");
          router.push(`/upload?assetGroupId=${assetGroup.id}`);
        }}
      ...
      <div className="library-body">
        ...
        <main className="workspace">
          ...
          {loading && <div className="data-state">正在加载数据库素材…</div>}
          {!loading && activeError && <div className="data-state is-error" role="alert">{activeError}</div>}
          {!loading && !activeError && data && <>
              ...
              <div className="asset-toolbar">
                ...
                <div className="asset-actions">
                  ...
                  <div className="column-switcher" aria-label="图片网格列数">
                    ...
                    {([4, 6, 8] as const).map((count) => <button key={count} type="button" aria-pressed={columns === count} onClick={() => setColumns(count)}>{count}</button>)}
                  </div>
                </div>
              </div>
